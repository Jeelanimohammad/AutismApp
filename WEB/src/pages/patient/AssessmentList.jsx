...
import { Play, Clock, CheckCircle, FileText, ChevronDown, ChevronUp, Download, Trash2 } from 'lucide-react';
...
export default function AssessmentList() {
  ...
  const handleDownloadReport = (asmtId, resultMessage, createdAt) => {
    const asmtDetails = details[asmtId];
    const asmtAdvice = advice[asmtId] || [];
    if (!asmtDetails) return;

    const reportContent = `
AUTISCREEN - ASSESSMENT REPORT
==============================
Patient Name: ${patient.name}
Patient ID: ${patient.patient_id}
Date Evaluated: ${createdAt}
Result Status: ${resultMessage}

SYMPTOM SURVEY RESPONSES
------------------------
${asmtDetails.responses.map(r => `${r.symptom_display_name || r.symptom_name}: ${r.response}`).join('\n')}

CLINICAL GUIDANCE & FEEDBACK
----------------------------
${asmtAdvice.length > 0 ? asmtAdvice.map(a => `[${a.created_at}] Dr. ${a.doctor_name}: ${a.advice_text}`).join('\n') : 'No clinical guidance posted yet.'}
`;
    const blob = new Blob([reportContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Autism_PatientReport_${patient.name}_${asmtId}.txt`;
